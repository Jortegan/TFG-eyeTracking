...
while cap.isOpened():
    ...
    for face_landmarks in results.multi_face_landmarks:
        ...
        if draw_gaze:
            ...
            # Draw on trail image
            gaze_point = tuple(np.ravel(l_gaze_axis[2]).astype(np.int32))
            cv2.circle(trail_img, (gaze_point[0] + trail_img_width // 2, gaze_point[1] + trail_img_height // 2), 2, (0, 0, 255), -1)
        ...
        if draw_gaze:
            ...
            # Draw on trail image
            gaze_point = tuple(np.ravel(r_gaze_axis[2]).astype(np.int32))
            cv2.circle(trail_img, (gaze_point[0] + trail_img_width // 2, gaze_point[1] + trail_img_height // 2), 2, (0, 0, 255), -1)
